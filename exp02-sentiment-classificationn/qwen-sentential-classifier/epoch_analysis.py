"""
训练轮数（Epochs）对模型准确率的影响分析 - Qwen版本
...
def train_multiple_epochs(train_texts, train_labels, val_texts, val_labels,
    ...
    print(f"\n最佳验证准确率: {best_val_accuracy:.4f} (Epoch {best_epoch})")
...
def plot_overfitting_analysis(history, output_path):
    ...
    plt.tight_layout()
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"✓ 过拟合分析图已保存到: {output_path}")
    plt.close()

def main():
    parser = argparse.ArgumentParser(description='分析训练轮数对模型准确率的影响')
