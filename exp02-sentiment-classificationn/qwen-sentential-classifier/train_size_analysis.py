...
def main():
    parser = argparse.ArgumentParser(description='分析训练集大小对模型准确率的影响')
    ...
    print(f"可用训练样本总数: {len(train_texts)}")
    ...
    for train_size in args.train_sizes:
        if train_size > len(train_texts):
            print(f"\n警告: 请求的训练集大小 {train_size} 超过可用数据 {len(train_texts)}")
        ...
        print(f"\n训练集大小={train_size} 的结果:")
        print(f"  最佳验证准确率:  {result['best_val_accuracy']:.4f}")
        print(f"  最终验证准确率:  {result['final_val_accuracy']:.4f}")
