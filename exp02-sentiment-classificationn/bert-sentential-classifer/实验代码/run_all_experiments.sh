...
# 检查是否有训练好的模型
if [ ! -f "sentiment_model.pth" ]; then
    echo -e "${YELLOW}警告: 未找到训练好的模型 sentiment_model.pth${NC}"
...
if [[ $REPLY =~ ^[Yy]$ ]]
then
    python train_size_analysis.py --train-sizes 500 1000 2000 5000 --epochs 3 --val-size 1000
else
    echo "跳过训练集大小分析"
fi
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]]
then
    python epoch_analysis.py --max-epochs 10 --train-size 5000 --val-size 1000 --test-size 1000
else
    echo "跳过训练轮数分析"
fi
echo ""
